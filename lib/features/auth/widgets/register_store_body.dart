...
class RegisterStoreBody extends StatefulWidget {
  const RegisterStoreBody({super.key});

  @override
  State<RegisterStoreBody> createState() => _RegisterScreenBodyState();
}
  ...
  Widget build(BuildContext context) {
    return BlocConsumer<RegisterCubit, RegisterState>(
      listenWhen: (previous, current) =>
          ...
          current is RegisterLoadingStore,
      listener: (context, state) {
        if (state is RegisterLoadingStore) {
          isloading = true;
        }
        if (state is RegisterSuccessStore) {
          isloading = false;
        ...
        if (state is RegisterErrorStore) {
          isloading = false;
              ...
              child: Column(
                ...
                children: [
                  ...
                  AppTextField(
                    labelText: "storeName",
                    controller:
                        context.read<RegisterCubit>().storeNameController,
                    keyboardType: TextInputType.text,
                    validator: (value) {
                      if (value == null || value.isEmpty) {
                        return 'Please enter your storeName';
                      }
                      return null;
                    },
                  ),
                  ...
                  AppTextField(
                    labelText: "hotLine",
                    controller: context.read<RegisterCubit>().hotLineController,
                    keyboardType: TextInputType.number,
                    validator: (value) {
                      if (value == null || value.isEmpty) {
                        return 'Please enter your hotLine';
                      }
                      return null;
                    },
                  ),
                  verticalSpace(30),
                  AppTextField(
                    labelText: "whatsappPhone",
                    controller:
                        context.read<RegisterCubit>().whatsappPhoneController,
                    keyboardType: TextInputType.number,
                    validator: (value) {
                      if (value == null || value.isEmpty) {
                        return 'Please enter your whatsappPhone';
                      }
                      return null;
                    },
                  ),
